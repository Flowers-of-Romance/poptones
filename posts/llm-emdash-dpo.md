...
出現数は全モデルの日本語instruct出力（75サンプル）の合算。em dash/en dashはゼロで、日本語出力ではモデルは長音符「ー」とハイフン「-」を使っている。英語のinstructモデルが使うem dashとは別の文字体系であり、英語のDPOで学習された「em dashを挿入句に使う」パターンは日本語出力には転移していない。

ではこれらのダッシュ類似文字自体がinstruct化で増幅されるか。base vs instructでper 1k morphemesを比較した。
