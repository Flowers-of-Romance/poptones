---
layout: post.vto
title: "LLMの文体について みたび: em dash注入実験"
---

<div class="post-content">

# LLMの文体について　みたび: em dash注入実験
...
- **SFTデータがtulu3由来**: em dashだけでなくtulu3の文体全体を学習している可能性がある。純粋にem dashのみを注入するには、Qwen2.5自身の出力にem dashを機械的に挿入した最小ペアでのSFTが必要だが、今回のデータ量では効果が不十分だった
- **因果の方向**: SFTはモデルの分布全体をシフトさせるため、「em dashがコロンを引き連れた」のか「tulu3の文体全体がコロンとem dashを同時に含んでいた」のかの完全な切り分けはできていない
- **訓練データの構造マーカー分布を未統制**: tulu3のem dashあり応答が、em dashなし応答と比べてコロン・セミコロンも多い可能性がある。つまりSFT後の変化はモデル内部の結合ではなく、単にデータの共起を学習しただけかもしれない

最後の点は重要で、これを切り分けるには訓練データ内のコロン・セミコロン頻度を統制した条件が必要。具体的には、em dashを多く含むがコロン・セミコロンは平均的なサンプルだけでSFTする実験。

## 次のステップ

1. **訓練データの句読点統制**: em dashは多いがコロン・セミコロンは平均的なサンプルでSFT → データの共起 vs モデル内部の結合を切り分け
2. **8Bモデルでの再現**: クラウドGPUで同じ実験を実行
3. **双方向テスト**: 逆にコロンを注入してem dashが増えるかを検証 → 結合の方向性を確認
